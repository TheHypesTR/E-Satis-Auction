...
import { auctionStatusLabel, formatMoney, getApiErrorMessage, getCurrentUserIdFromToken, isActiveAuction, isAuctionPaymentPending, makeIdempotencyKey, requireAuth } from '../../services/apiUtils';
import type { AuctionBidDto, AuctionDetailDto, AuctionWinnerDto, PaymentInitiationDto } from '../../types/commerce';
...
export default function UserAuctionDetail() {
  ...
  const load = async () => {
    ...
    setError('');
    try {
      const [detail, bidList, winnerInfo] = await Promise.all([
        commerceApi.getAuction(id),
        commerceApi.getAuctionBids(id, 20),
        commerceApi.getAuctionWinner(id).catch(() => null),
      ]);
      setAuction(detail);
      setBids(bidList);
      setWinner(winnerInfo);
      setAmount(String(detail.minimumNextBid));
  ...
  const initiateWinnerPayment = async () => {
    ...
    const winnerUserId = winner?.winningUserId ?? auction?.winningUserId ?? null;
    if (!winnerUserId || currentUserId !== winnerUserId) { setError('Kazanan kullanıcı değilseniz açık artırma ödeme akışını başlatamazsınız. Backend bu kontrolü zorunlu tutar.'); return; }
    setActionLoading(true); setError(''); setMessage('');
    try {
      const result = await commerceApi.initiateAuctionPayment(id, makeIdempotencyKey('auction_payment'));
      setPayment(result);
      setMessage('Açık artırma ödeme simülasyonu başlatıldı. Gerçek ödeme sağlayıcısı yoktur; state machine backend’de simüle edilir.');
    } catch (err) { setError(getApiErrorMessage(err, 'Kazanan ödeme akışı başlatılamadı.')); }
    finally { setActionLoading(false); }
  };
  ...
  const failPayment = async () => {
    ...
    try {
      ...
      setPayment(null);
      setMessage('Ödeme simülasyonu başarısız işaretlendi. Backend auction payment fail/expire akışında stok release davranışını uygular.');
      await load();
  ...
  const currentUserId = getCurrentUserIdFromToken();
  const winnerUserId = winner?.winningUserId ?? auction.winningUserId ?? null;
  const isWinner = Boolean(currentUserId && winnerUserId && currentUserId === winnerUserId);

  return <div className="user-page"><button className="btn btn-ghost" style={{ marginBottom: 28, gap: 6 }} onClick={() => navigate(-1)}><ArrowLeft size={15} /> Geri</button><div className="user-detail-grid"><div><div className="user-detail-img" style={{ background: 'linear-gradient(135deg, #7c3aed 0%, #f59e0b 100%)' }}><Gavel size={84} color="rgba(255,255,255,0.55)" /></div><div className="glass-card" style={{ marginTop: 20, padding: 18 }}><h3 style={{ marginBottom: 12 }}>Son Teklifler</h3>{bids.length === 0 ? <p style={{ color: 'var(--text-muted)' }}>Henüz teklif yok.</p> : bids.map(b => <div key={b.id} style={{ display: 'flex', justifyContent: 'space-between', padding: '10px 0', borderBottom: '1px solid var(--glass-border)' }}><span style={{ color: 'var(--text-muted)' }}>{new Date(b.createdAt).toLocaleTimeString('tr-TR')}</span><strong>{formatMoney(b.amount, auction.currency)}</strong></div>)}</div></div><div><span className="badge badge-amber" style={{ marginBottom: 12 }}>{auctionStatusLabel[String(auction.status)] ?? String(auction.status)}</span><h1 style={{ fontSize: '1.8rem', fontWeight: 700, marginBottom: 8 }}>{auction.productName}</h1><code style={{ color: 'var(--text-muted)' }}>{auction.sku}</code><div className="glow-divider" style={{ margin: '24px 0' }} /><div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 24 }}><Metric label="Güncel fiyat" value={formatMoney(auction.currentPrice, auction.currency)} /><Metric label="Minimum sonraki teklif" value={formatMoney(auction.minimumNextBid, auction.currency)} /><Metric label="Bitiş" value={new Date(auction.endsAt).toLocaleString('tr-TR')} /><Metric label="Anti-snipe" value="Son 5 dk teklif gelirse +5 dk" /></div>{error && <div className="error-banner" style={{ marginBottom: 16 }}><AlertCircle size={15} /> {error}</div>}{message && <div style={{ marginBottom: 16, padding: '12px 16px', color: '#6ee7b7', background: 'rgba(16,185,129,0.08)', borderRadius: 10, border: '1px solid rgba(16,185,129,0.15)' }}>{message}</div>}{active && <div className="glass-card" style={{ padding: 18, marginBottom: 18 }}><label className="form-label">Teklif Tutarı</label><div style={{ display: 'flex', gap: 10 }}><input className="form-input" type="number" value={amount} onChange={e => setAmount(e.target.value)} min={auction.minimumNextBid} step="0.01" /><button className="btn btn-primary" onClick={placeBid} disabled={actionLoading}>Teklif Ver</button></div><p style={{ color: 'var(--text-muted)', fontSize: '0.78rem', marginTop: 10 }}>Frontend minimum teklif ön kontrolü yapar; asıl kontrol ve idempotency backend’dedir.</p></div>}{paymentPending && <div className="glass-card" style={{ padding: 18 }}><h3 style={{ marginBottom: 10 }}>Kazanan Ödeme Akışı</h3><p style={{ color: 'var(--text-muted)', fontSize: '0.88rem', marginBottom: 10 }}>Bu bölüm simülasyon/demo ödeme akışıdır. Gerçek sanal POS entegrasyonu göstermez.</p><p style={{ color: 'var(--text-muted)', fontSize: '0.82rem', marginBottom: 14 }}>Kazanan kullanıcı: {winnerUserId ?? 'henüz bilinmiyor'}</p>{!isWinner && <p style={{ color: '#fbbf24', fontSize: '0.84rem', marginBottom: 14 }}>Ödeme başlatma aksiyonu sadece kazanan kullanıcı için aktiftir; backend forbidden kontrolü de uygular.</p>}{!payment ? <button className="btn btn-primary" onClick={initiateWinnerPayment} disabled={actionLoading || !isWinner}><Clock size={16} /> Kazanan Ödeme Simülasyonunu Başlat</button> : <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}><button className="btn btn-primary" onClick={confirmPayment} disabled={actionLoading}><CheckCircle2 size={16} /> Demo Confirm</button><button className="btn btn-ghost" style={{ color: '#f87171', borderColor: 'rgba(248,113,113,0.3)' }} onClick={failPayment} disabled={actionLoading}><XCircle size={16} /> Demo Fail</button></div>}</div>}</div></div></div>;
}
